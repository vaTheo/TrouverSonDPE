...
import { ApiResponse, ResultItemDPE } from './externalApi';
import axiosInstanceWithUserAdgent from 'src/server/utils/axiosInstance';
import { KEYSTOKEEPDPE } from './keysToKeep';
import { filterObjectKeys } from 'src/server/utils/utilities';
import { DPEAllData } from './DPE';
...
export class DpeService {
    ...
    try {
      ...
      if (endpoint == 'dpe-france' || endpoint == 'dpe-tertiaire') {
        ...
        const response = await axiosInstanceWithUserAdgent.get(
          `${URLAPI}geo_agg?geo_distance=${long}:${lat}:${aroundMeters}`,
        );
        data = response.data;
      } else {
        const response = await axiosInstanceWithUserAdgent.get(
  ...
  getDPERates(DPEAllData: DPEAllData): DPEAllData {
    DPEAllData.rateDPE =
      (this.RateDPE(DPEAllData.DPEHabitatExistant) +
        this.RateDPE(DPEAllData.DPEHabitatNeuf) +
        this.RateDPE(DPEAllData.DPETertiaire) +
        this.RateDPE(DPEAllData.DPETertiaireAvant2021) +
        this.RateDPE(DPEAllData.DPEHabitatExistantAvant2021)) /
      5;
